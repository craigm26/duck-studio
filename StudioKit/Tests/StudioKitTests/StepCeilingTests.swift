...
final class StepCeilingTests: XCTestCase {
    func testTheShippedCeilingCarriesItsProvenanceAndClearsNothingReliably() {
        ...
        XCTAssertEqual(c.rounds, 6)
        XCTAssertEqual(c.episodes, 55_000)
        XCTAssertFalse(c.criterion.isEmpty)
        XCTAssertTrue(c.evidence.contains("r6"))
    ...
    func testARiseAboveEverythingGetsTheSearchCountInWords() {
        ...
        XCTAssertTrue(said.contains("Nothing has got up 90 mm or taller in any of roughly 55,000 searched attempts over 6 rounds"), said)
