...
7. D-7 DuckSimulation.State — a readable and writable struct (jointPositions, previousTargets, lastAction, tickCount) plus init(walk:stand:state:). Everything is private today, so Microduck Studio cannot inject a state, cannot pin an observation slot mid-loop, and cannot restart two policies from an identical initial condition for the closed-loop divergence diff.
...
10. NOT needed, stated so it does not get dragged in: PhoneSight. Microduck Studio's AR needs a horizontal plane and nothing more; LiDAR capability tiering would add an ARKit dependency to a package whose entire value is that it builds and tests on Linux aarch64.
